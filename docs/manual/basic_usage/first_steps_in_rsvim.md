...
- [`Rsvim.buf.current()`](/docs/next/api/rsvim/interfaces/RsvimBuf#current) returns current buffer ID
- [`Rsvim.buf.writeSync()`](/docs/next/api/rsvim/interfaces/RsvimBuf#writesync) writes the buffer (specified by ID) to file system synchronously
  :::
