...
<Tabs>
  <TabItem value="linux" label="linux" default>
    ```bash
    curl -fsSL https://github.com/rsvim/rsvim/releases/tag/nightly/download/rsvim-x86_64-unknown-linux-gnu.tar.gz | tar zxf -
    ```
  </TabItem>
  <TabItem value="mac" label="mac">
    ```bash
    curl -fsSL https://github.com/rsvim/rsvim/releases/tag/nightly/download/rsvim-aarch64-apple-darwin.tar.gz | tar zxf -
    ```
  </TabItem>
  <TabItem value="windows" label="windows">
    ```powershell
    Invoke-WebRequest -Uri https://github.com/rsvim/rsvim/releases/tag/nightly/download/rsvim-x86_64-pc-windows-msvc.zip -OutFile rsvim.zip
    Expand-Archive -Path rsvim.zip
    ```
  </TabItem>
</Tabs>
...
For latest updates, build the `main` branch:
